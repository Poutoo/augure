  ...
  return (
    ...
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center">
      ...
      <div className={panelClass}>
        ...
        {/* ── Content ── */}
        <div className="flex-1 bg-white rounded-t-[32px] -mt-6 relative z-20 flex flex-col overflow-hidden">

          {/* Onglets */}
          <div className="flex-shrink-0 flex border-b border-gray-100 px-6 pt-5">
            {(['detail', 'comments'] as const).map(tab => (
              <button
                key={tab}
                onClick={() => setActiveTab(tab)}
                className={`pb-3 px-3 mr-2 font-syne font-bold text-sm border-b-2 transition-colors ${
                  activeTab === tab
                    ? 'border-[var(--color-text-dark)] text-[var(--color-text-dark)]'
                    : 'border-transparent text-gray-400 hover:text-gray-600'
                }`}
              >
                {tab === 'detail' ? 'Détail' : 'Commentaires'}
              </button>
            ))}
          </div>
